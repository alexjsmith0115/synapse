...
### Fixed
- **Center node kind no longer hardcoded to Method** — `neighborhoodToElements` now uses `data.kind || 'Method'` instead of the hardcoded `'Method'` literal; the actual symbol kind from the API response is reflected in the graph (regression test added)
