...
- **Webviewer frontend** — updated tool sidebar to match v3.0 surface: removed deprecated `get_hierarchy`, replaced `scope` select with `members_only` checkbox on `get_context_for`, added `read_symbol`, `assess_impact`, and `find_implementations` tools
