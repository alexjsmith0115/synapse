...
# ---------------------------------------------------------------------------
# Regression: top-level arrow functions with zero children are indexed
# (ts-dead-code-false-positives)
# ---------------------------------------------------------------------------

class TestArrowFunctionZeroChildren:
    """Regression: TSServer reports some top-level arrow functions with children=[].

    When an arrow function body contains only if/return statements and no nested
    const/property symbols, TSServer emits ``children: []``.  The old code used
    ``raw.get("children")`` as a truthiness guard, which treated an empty list as
    falsy and silently dropped these functions — causing false positives in dead
    code detection.

    The fix uses tree-sitter to confirm the variable declarator's value is an
    arrow_function before promoting it to a Method node.
    """

    def _make_adapter(self, tmp_path) -> object:
        from synapps.lsp.typescript import TypeScriptLSPAdapter
        from unittest.mock import MagicMock
        return TypeScriptLSPAdapter(MagicMock(), str(tmp_path))

    def _make_raw_const(self, name: str, line: int, end_line: int,
                        children: list | None = None) -> dict:
        raw: dict = {
            "name": name,
            "kind": 14,  # Constant
            "location": {"range": {
                "start": {"line": line, "character": 13},
                "end": {"line": end_line, "character": 1},
            }},
            "selectionRange": {
                "start": {"line": line, "character": 13},
                "end": {"line": line, "character": 13 + len(name)},
            },
        }
        if children is not None:
            raw["children"] = children
    ...
    def test_is_arrow_function_at_line_detects_arrow_fn(self, tmp_path: Path) -> None:
        """_is_arrow_function_at_line returns True for a const arrow function."""
        import tree_sitter_typescript as ts_ts
        from tree_sitter import Language, Parser
        from synapps.lsp.typescript import _is_arrow_function_at_line

        lang = Language(ts_ts.language_typescript())
        parser = Parser(lang)
        source = b"export const formatDueDate = (s: string): string => {\n  return s;\n};\n"
        tree = parser.parse(source)

        assert _is_arrow_function_at_line(tree, 0) is True

    def test_is_arrow_function_at_line_rejects_plain_constant(self, tmp_path: Path) -> None:
        """_is_arrow_function_at_line returns False for a plain string constant."""
        import tree_sitter_typescript as ts_ts
        from tree_sitter import Language, Parser
        from synapps.lsp.typescript import _is_arrow_function_at_line
        ...
        lang = Language(ts_ts.language_typescript())
        parser = Parser(lang)
        source = b"export const API_URL = 'https://example.com';\n"
        tree = parser.parse(source)

        assert _is_arrow_function_at_line(tree, 0) is False

    def test_is_arrow_function_at_line_rejects_type_alias(self, tmp_path: Path) -> None:
        """_is_arrow_function_at_line returns False for a type alias (no variable_declarator)."""
        import tree_sitter_typescript as ts_ts
        from tree_sitter import Language, Parser
        from synapps.lsp.typescript import _is_arrow_function_at_line
        ...
        lang = Language(ts_ts.language_typescript())
        parser = Parser(lang)
        source = b"export type StatusToken = 'a' | 'b' | 'c';\n"
        tree = parser.parse(source)

        assert _is_arrow_function_at_line(tree, 0) is False

    def test_traverse_promotes_zero_children_arrow_fn_to_method(self, tmp_path: Path) -> None:
        """Regression: top-level const arrow fn with children=[] is indexed as METHOD.

        This is the core regression test for ts-dead-code-false-positives.
        TSServer reports ``getSemanticStatus`` and similar functions with children=[]
        when their bodies contain only if/return statements.  They must be indexed
        as Method nodes so callers produce CALLS edges and dead-code detection
        does not falsely flag them.
        """
        ts_file = tmp_path / "status.ts"
        ts_file.write_text(
            "export const getSemanticStatus = (item: any, cls: string): string => {\n"
            "  if (item.status === 'Completed') return 'completed';\n"
            "  return 'open';\n"
            "};\n"
        )

        from synapps.lsp.typescript import TypeScriptLSPAdapter, _parse_ts_file
        from unittest.mock import MagicMock

        adapter = TypeScriptLSPAdapter(MagicMock(), str(tmp_path))
        ts_tree = _parse_ts_file(str(ts_file))
        assert ts_tree is not None

        raw = self._make_raw_const("getSemanticStatus", line=0, end_line=3, children=[])
        result: list = []
        adapter._traverse(raw, str(ts_file), parent_full_name=None, result=result, ts_tree=ts_tree)

        assert len(result) == 1
        assert result[0].name == "getSemanticStatus"
        assert result[0].kind == SymbolKind.METHOD
        assert result[0].signature == "const_function"

    def test_traverse_skips_plain_constant_with_zero_children(self, tmp_path: Path) -> None:
        """Plain constants (non-function) with children=[] are still skipped."""
        ts_file = tmp_path / "config.ts"
        ts_file.write_text("export const API_URL = 'https://example.com';\n")
        ...
        raw = self._make_raw_const("API_URL", line=0, end_line=0, children=[])
        result: list = []
        adapter._traverse(raw, str(ts_file), parent_full_name=None, result=result, ts_tree=ts_tree)

        assert result == []

    def test_traverse_skips_type_alias_with_zero_children(self, tmp_path: Path) -> None:
        """Type aliases (kind=13, no variable_declarator) with children=[] are still skipped."""
        ts_file = tmp_path / "types.ts"
        ts_file.write_text("export type StatusToken = 'a' | 'b' | 'c';\n")
        ...
        # TSServer reports type aliases as kind=13 (Variable)
        raw = {
            "name": "StatusToken",
            "kind": 13,
            "location": {"range": {
    ...
    def test_traverse_without_ts_tree_still_skips_zero_children(self, tmp_path: Path) -> None:
        """When ts_tree is None (e.g. parse failure), zero-children symbols are safely skipped."""
        from synapps.lsp.typescript import TypeScriptLSPAdapter
        from unittest.mock import MagicMock

        adapter = TypeScriptLSPAdapter(MagicMock(), str(tmp_path))
        raw = self._make_raw_const("someFunc", line=0, end_line=5, children=[])
        result: list = []
        adapter._traverse(raw, str(tmp_path / "f.ts"), parent_full_name=None, result=result, ts_tree=None)
